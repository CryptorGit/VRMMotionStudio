...
export function ensureShapeKeyCache(root, { force = false } = {}) {
  ...
  try {
    root.traverse(obj => {
      ...
      // morphTargetDictionary縺九ｉ隱ｭ縺ｿ霎ｼ縺ｿ
      for (const [name, index] of Object.entries(dict)) {
        if (!Number.isInteger(index)) continue
        if (index < 0 || index >= influences.length) continue // 譛牙柑縺ｪ遽・峇縺ｮ縺ｿ
      ...
      // geometry.morphAttributes縺九ｉ繧ら｢ｺ隱搾ｼ・ict縺ｫ縺ｪ縺・ｴ蜷医・繝輔か繝ｼ繝ｫ繝舌ャ繧ｯ・・
      const geometry = obj.geometry
      if (geometry?.morphAttributes && typeof geometry.morphAttributes === 'object') {
...
const SHAPEKEY_GROUP_DEFINITIONS = [
  { key: 'eyes', label: '逶ｮ繝ｻ迸ｳ', matchers: [/\beye/i, /blink/, /wink/, /look(?!at)/, /gaze/, /pupil/, /iris/, /逹ｨ/, /隕也ｷ・, /逶ｮ邱・, /豕ｨ隕・, /^逶ｮ/, /迸ｳ/, /繧ｦ繧｣繝ｳ繧ｯ/, /縺ｾ縺ｰ縺溘″/, /^隕・] },
  { key: 'brow', label: '逵・, matchers: [/brow/, /mayu/, /逵・] },
  { key: 'mouth', label: '蜿｣繝ｻ蜿｣蜈・, matchers: [/mouth/, /\blip/i, /jaw/, /tongue/, /teeth/, /^[aiueo]$/i, /viseme/, /vrc\.v_/, /^蜿｣/, /^縺・, /^縺・, /^縺・, /^縺・, /^縺・, /^繧・, /^隨・, /smile/, /frown/, /闊・, /豁ｯ/] },
  { key: 'face', label: '鬆ｬ繝ｻ陦ｨ諠・, matchers: [/face/, /cheek/, /nose/, /鮠ｻ/, /鬆ｬ/, /tear/, /豸・, /縺ｻ縺・, /鬆ｰ/, /陦ｨ諠・] },
  { key: 'hair', label: '鬮ｪ', matchers: [/hair/, /鬮ｪ/, /bang/, /蜑埼ｫｪ/] },
  { key: 'body', label: '霄ｫ菴・, matchers: [/body/, /bust/, /breast/, /arm/, /hand/, /finger/, /leg/, /foot/, /toe/, /shoulder/, /spine/, /waist/, /hip/, /neck/, /鬆ｭ/, /閧ｩ/, /閻・, /謇・, /雜ｳ/, /謖・, /閭ｸ/, /菴・] }
]

const SHAPEKEY_GROUP_ORDER = ['eyes', 'brow', 'mouth', 'face', 'hair', 'body', 'other']

const DEFAULT_UNNAMED_LABEL = '蜷咲ｧｰ譛ｪ險ｭ螳・
...
export function describeShapeKeys(root, { includeExpressions = false, exclude = [] } = {}) {
  ...
  for (const rawName of names) {
    ...
    if (count > 0) {
      const marker = duplicateSuffix(count) || '蛻･'
      display = `${friendly}・・{marker}・荏
    }
    group.items.push({
      name: rawName,
      label: display,
