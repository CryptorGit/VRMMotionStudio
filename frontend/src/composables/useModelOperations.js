...
export function useModelOperations({
  ...
  async function restoreCachedModel() {
    const saved = await modelCache.loadCachedFiles()
  ...
  async function removeModel(index) {
    ...
    try {
      ...
      if (models.value.length === 0) {
        await modelCache.deleteCachedFiles()
      } else {
        await modelCache.deleteCachedFiles(index)
        await modelCache.cacheFiles(models.value.map(m => m.files || []))
      }
    } catch (e) {
